...
class Settings(Screen):
    ...
    # НОВИЙ МЕТОД: Вмикання/вимикання фонової музики
    def toggle_music(self, state):
        game_screen = self.parent.get_screen('game')
        
        # Оновлюємо властивість is_music_on в ClickerApp
        app.is_music_on = state == 'down'
        
        if app.is_music_on:
            # Граємо музику, якщо вона включена і не відтворюється
            if game_screen.back_sound.state != 'play':
                game_screen.back_sound.play()
        else:
            # Зупиняємо музику
            game_screen.back_sound.stop()
...
class ClickerApp(App):
    ...
    FISHES = {
    'hamster1':
        {'source': 'assets/images/hamster_01.png', 'hp': 15},
    'hamster2':
        {'source': 'assets/images/hamster_02.png', 'hp': 20},
    'hamster3':
        {'source': 'assets/images/hamster_03.png', 'hp': 25}
    }
